...
interface TrendVisibilityDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  hiddenTrendKeys: string[];
  onToggle: (key: string) => void;
}

export function TrendVisibilityDialog({
  open,
  onOpenChange,
  hiddenTrendKeys,
  onToggle,
}: TrendVisibilityDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Trend-Graphen verwalten</DialogTitle>
          <DialogDescription>
            Wähle aus, welche Graphen in der Trends-Ansicht angezeigt werden.
          </DialogDescription>
        </DialogHeader>
              ...
              <AccordionContent>
                <div className="divide-y">
                  {group.metrics.map((metric) => {
                    ...
                    const isVisible = !hiddenTrendKeys.includes(metric.key);
